...
@Injectable()
export class Image_Suborder_Add_Service {
  constructor(
    @InjectRepository(Image_Suborder_Add)
    private imageSubOrderRepository: Repository<Image_Suborder_Add>,
  ) {}
  create(createImageSoDto: Image_Suborder_AddDto): Promise<Image_Suborder_Add> {
    try {
  ...
  async findAllBySubOrderId(subOrderId: number) {
    try {
      const result = await this.imageSubOrderRepository.find({
        where: { image_Suborder_add_subOrder_add_id: subOrderId },
      });
      if (result) {
        return result;
      } else {
  ...
  findOne(id: number) {
    return this.imageSubOrderRepository.findOneBy({
      image_Suborder_add_id: id,
    });
  }

  async update(updateImageGoDto: Image_Suborder_AddDto): Promise<boolean> {
    try {
      const response = await this.imageSubOrderRepository.update(
        updateImageGoDto.image_Suborder_add_id,
        updateImageGoDto,
      );
